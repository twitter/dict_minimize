...
@given(dictionaries(text(), tuples(np_float_arrays, torch_float_dtypes), min_size=1))
...
def validate_solution(x0_dict, x_sol, lb=None, ub=None):
    ...
    for kk in x0_dict.keys():
        ...
        assert (lb is None) or np.all(to_np(lb[kk]) <= to_np(x_sol[kk]))
        assert (ub is None) or np.all(to_np(x_sol[kk]) <= to_np(ub[kk]))
